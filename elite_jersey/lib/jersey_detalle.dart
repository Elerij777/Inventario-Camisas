...
class _JerseyDetalleState extends State<JerseyDetalle> {
  ...
  Future<void> _updateJersey() async {
    ...
    try {
      await DBHelper.update(updatedJersey);
  ...
  Future<void> _deleteJersey() async {
    try {
      await DBHelper.delete(widget.jersey.id!);
      _showSuccessDialog('Jersey eliminado exitosamente');
      Navigator.pop(context); // Volver a la pantalla anterior
    } catch (e) {
      _showErrorDialog('Error: ${e.toString()}');
    }
